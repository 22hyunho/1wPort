...
void image::alphaRender(HDC hdc, BYTE alpha)
{
	...
	{
		//이미지를 먼저 복사합시다
		BitBlt(_blendImage->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, // 복사해와서 뿌릴지점 x,y
			hdc, _imageInfo->x, _imageInfo->y, SRCCOPY);						 // 복사해올곳 x,y

		//복사해온 이미지에서 특정칼라(보통 마젠타)를 제거
		GdiTransparentBlt(_blendImage->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height,	 //대상은 블랜드 memdc에 있는 복사하려는 이미지 크기만큼
		_imageInfo->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, _transColor);		 //역시 복사해올곳은 복사하려는 이미지정보 memdc에서 가로,크기만큼, 제거할 칼라값
	...
	{
		AlphaBlend(hdc, _imageInfo->x, _imageInfo->y, _imageInfo->width, _imageInfo->height,	//backDC에 뿌려준다(2)
			_imageInfo->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, _blendFunc);		//알파 적용을 해서(1)
	}
}
